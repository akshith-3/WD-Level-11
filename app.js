...
const { admin, election, question, options } = require("./models");
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const ques = await await question.retriveQuestions(req.params.id);
      res.render("endlaunch", {
        title: "Election Page",
        id: req.params.id,
        ele,
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      const question1 = await question.createQuestion({
...
app.get(
  ...
  async (req, res) => {
    const ques = await await question.retriveQuestions(req.params.id);
    const election1 = await election.findByPk(req.params.id);
    if (req.accepts("html")) {
